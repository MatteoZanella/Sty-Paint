...
def local2global(params1, params2, order, x_id_list, y_id_list, render_size_x, render_size_y, patch_x, patch_y, original_size=256):

    out = []
    params1 = params1.cpu().numpy()
    params2 = params2.cpu().numpy()
    params = np.concatenate((params1, params2))
    for idx in order:
        x,y,w,h,theta,r,g,b = params[idx, :]
        x_id = x_id_list[idx]
        y_id = y_id_list[idx]
        xg = (x * patch_x + patch_x * x_id) / render_size_x
        yg = (y * patch_y + patch_y * y_id) / render_size_y
        wg = (w * patch_x) / render_size_x
        hg = (h * patch_y) / render_size_y

        out.append(np.array([xg, yg, wg, hg, theta, r, g, b ]))
...
def get_single_layer_lists(param, decision, ori_img, render_size_x, render_size_y, h, w, meta_brushes, dilation,
    ...
    """
    # this outputs the rendered strokes. without color, then they threshold to obtain the alpha mask (they have spurious values because of interpolation I guess)
    ...
    patch_y = 4 * render_size_y // 5  # resotre to the original dimension, without the 1.25
    ...
    # This indexing is a sort of cropping, they discard a border of size 64 left/right/up/down
...
def stroke_net_predict(img_patch, result_patch, patch_size, net_g, stroke_num) :
    ...
    # TODO: they sample the color from the reference image, based on the location of the predicted storke
    color = nn.functional.grid_sample(img_temp, 2 * grid - 1, align_corners=False).reshape([
        img_patch.shape[0], stroke_num, 3])
    stroke_param = paddle.concat([shape_param, color], axis=-1)  # these are clean stroke params

    param = stroke_param.reshape([-1, 8])
    decision = stroke_decision.reshape([-1]).astype('bool')
    param[:, :2] = param[:, :2] / 1.25 + 0.1
    param[:, 2 :4] = param[:, 2 :4] / 1.25  # 1.25 maybe is due to the resize by 1.25
...
def render_serial(original_img, net_g, meta_brushes) :
    ...
    with paddle.no_grad() :
        ...
        for layer in range(0, K + 1) :
            ...
            # clean_strokes = local2global(wA_params,
            #                              wB_params,
            #                              order=sort_list,
            #                              x_id_list=wA_xid_list + wB_xid_list, y_id_list=wA_yid_list + wB_yid_list,
            #                              render_size_x=render_size_x, render_size_y=render_size_y,
            #                              patch_x=patch_x, patch_y=patch_y)
            #
            # all_strokes.append(clean_strokes)
            sample = 0
            ...
            for ii in sort_list :
                ...
                all_sample.append((final_result.numpy().squeeze().transpose([1, 2, 0]) * 255.0).astype(
                        np.uint8))
                if sample in samples :
                    saveframe = (final_result.numpy().squeeze().transpose([1, 2, 0])[:, :, : :-1] * 255).astype(
                        np.uint8)
