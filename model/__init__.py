from .modeltype import GAN, VAE_GAN, VAE, GAN_Image, VAE_GAN_Image
...
def build_model(config) :
    ...
    elif config["model"]["model_type"] == "patch-vae-gan":
        model = VAE_GAN_Image.VAEGANModel(config)
    elif config["model"]["model_type"] == "gan":
        model = GAN.GANModel(config)
    elif config["model"]["model_type"] == "patch_gan":
        model = GAN_Image.GANModel(config)
    else:
